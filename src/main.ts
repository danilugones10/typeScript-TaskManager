...
const TASKS = [
  ...
  function createTaskElement(task) {
    ...
    taskElement.querySelector('input').onchange = e => {
      const newTask = {...task};
      newTask.done = (<HTMLInputElement>e.target).checked;
    ...
    (<HTMLLIElement>taskElement.querySelector('span.material-icons')).onclick = () => {
  ...
  filterButtons
    .forEach(button => (<HTMLButtonElement>button).onclick = (event) => {
      const element = (<HTMLButtonElement>event.target);
      filterButtons.forEach(button => element.disabled = false);
